...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['9.0', '8.10', '8.0'])
            self.android_model = random.choice(['Y2'])
            self.useragent = ('Mozilla/5.0 (Linux; U; Android {}; Redmi {} Build/{}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/71.0.3578.99 UCBrowser/{} Mobile Safari/537.36'.format(self.android_version, self.android_model, self.build, self.browser_version))
